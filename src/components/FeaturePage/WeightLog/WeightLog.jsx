...
import { calculateBMR, getActivityFactor } from "../../FeaturePage/Food/CalculateBMR"; // Ensure correct path
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            ...
            try {
                ...
                const initialBMR = calculateBMR(userData.gender, userData.original_weight, userData.feet * 12 + userData.inches, userData.age);
        ...
        if (!isNaN(newEntryWeight) && id) {
            try {
                ...
                if (response.ok) {
                    ...
                    const updatedBMR = calculateBMR(userGender, newEntryWeight, heightFeet * 12 + heightInches, userAge);
      ...
      {showInputs && (
        <>
          ...
          <div className="bmi-display">
            <div>
              <p>
              <p>Daily Target Calories: {totalDailyCalories} kcal</p>
              </p>
            </div>
            <p>
              Current BMI: {BMI.toFixed(2)} ({getBMICategory(BMI)})
            </p>
            {BMI > 24.9 && (
              <p>
                Your BMI indicates that you might be overweight. Consider
                consulting a healthcare professional for advice.
              </p>
            )}
            {BMI < 18.5 && (
              <p>
                Your BMI indicates that you might be underweight. Consider
                consulting a healthcare professional for advice.
              </p>
            )}
          </div>
